...
exports.getAllImapEmailsLabel = async function(req, res) {
  try {
      ...
      var decodedData = req.decoded.data;
      createdata = await ImapEmails.findAll({ where: { customer_id: decodedData.id }, order: [['id','DESC']]});
      createdata = await sequelize.query(`select a.*, (select count(id) from imapEmails b where b.x_gm_thrid=a.x_gm_thrid) as count from imapEmails a where a.customer_id = ${decodedData.id} and a.x_gm_label='${req.params.label}'`, { type: QueryTypes.SELECT });
