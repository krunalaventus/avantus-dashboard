...
import { login,signup,adminLogin, getUsers, getUser, createUser, updateUser, deleteUser, searchUsers, loginCustomer, loginUsingToken } from '../controllers/users.controller';

UserRoutes.get('/', checkToken, getUsers);
UserRoutes.get('/search', checkToken, searchUsers);
UserRoutes.get('/:id', checkToken, getUser);
UserRoutes.get('/auth/access-token', checkToken, loginUsingToken);
